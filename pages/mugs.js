...
for(let products of product){
    if(products.title in mugs){
              if(!mugs[products.title].color.includes(products.color) &&products.availableQ>0){
                      mugs[products.title].color.push(products.color)
                      mugs[products.title].size.push(products.size)
              }
    }
    else{
        mugs[products.title]=JSON.parse(JSON.stringify(products))
        if(products.availableQ>0){
            mugs[products.title].color=[products.color]
            mugs[products.title].size=[products.size]
        }
    }
}
  return {
    
    props: {product: JSON.parse(JSON.stringify(mugs))}, // will be passed to the page component as props
  }
}