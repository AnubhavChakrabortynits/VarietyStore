...
export default function stickers({product}) {
  return (
    <div>
...
for(let products of product){
    if(products.title in stickers){
              if(!stickers[products.title].color.includes(products.color) &&products.availableQ>0){
                      stickers[products.title].color.push(products.color)
                      stickers[products.title].size.push(products.size)
              }
    }
    else{
        stickers[products.title]=JSON.parse(JSON.stringify(products))
        if(products.availableQ>0){
            stickers[products.title].color=[products.color]
            stickers[products.title].size=[products.size]
        }
    }
}
  return {
    
    props: {product: JSON.parse(JSON.stringify(stickers))}, // will be passed to the page component as props
  }
}